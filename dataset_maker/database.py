...
class DatabaseSor(Database):
    ...
    def add_translation(self, sentence_id, text, is_training):
        ...
        try:
            cursor.execute(
                f"INSERT INTO SorSentence (sentence_id, sentence_text, train) VALUES (?, ?, ?);",
...
class DatabaseIta(Database):
    ...
    def reset_working_status(self):
        ...
        try:
            cursor.execute(
                f"UPDATE ItaSentence SET status = ? WHERE status <> 0", (NOT_DONE,)
           )
        finally:
            cursor.close()
        return self.conn
    ...
    def add_entry(self, sentence_id, text, is_training):
        ...
        try:
            cursor.execute(
                f"INSERT INTO ItaSentence (sentence_id, sentence_text, train) VALUES (?, ?, ?);",
                       (sentence_id, text, is_training)
           )
        finally:
            cursor.close()
    ...
    def get_next_batch_items(self, batch_size: int):
        cursor = self.get_cursor()
        try:
            cursor.execute(f"SELECT sentence_id, sentence_text, train FROM ItaSentence WHERE status={NOT_DONE} LIMIT ?;", (batch_size,))
    ...
    def update_batch_job(self, sentence_ids, status):
        ...
        try:
            placeholders = ','.join(['?'] * len(sentence_ids))
            cursor.execute(f"UPDATE ItaSentence SET status = ? WHERE sentence_id IN ({placeholders}) LIMIT ?", (status, *sentence_ids, len(sentence_ids)))
            return self.conn
        except Exception as e:
