...
def parse_parquet_dataset():
    # If you want to use the dataset immediately and efficiently stream the data as you iterate over the dataset
    def load_samples():
        from datasets import load_dataset
        fw = load_dataset(
            "parquet",  # format
            data_files="./data/fineweb-2/data/ita_Latn/test/*.parquet",  # point to local files
            split="train",
...
def load_dataset(dir_path: str = './data/fineweb-2/data/ita_Latn') -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if not path.exists(path.join(dir_path, 'train.jsonl')) or not path.exists(path.join(dir_path, 'test.jsonl')):
        with zipfile.ZipFile(path.join(dir_path, 'ita_ds.zip'), 'r') as zip_ref:
            zip_ref.extractall(dir_path)

    with(open(path.join(dir_path, 'train.jsonl'), "r", encoding="utf-8")) as f:
        train_d = [json.loads(line) for line in f.readlines()]
    with(open(path.join(dir_path, 'test.jsonl'), "r", encoding="utf-8")) as f:
